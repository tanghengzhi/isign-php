...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressIco=$("<img/>");
        progressIco.css("width","40px");
        progressIco.css("height","40px");
        progressIco.attr("isLoad","0");

        var progressBr=$("<br/>");

        var progressText = $("<td/>");
        progressText.append(progressIco);
        progressText.append(progressBr);
        progressText.addClass('progressName').append(file.name);
...
FileProgress.prototype.setComplete = function (up, info) {
    ...
    if (res.url) {
        ...
        str = "<div><strong>下载链接:</strong><a href=" + url + " target='_blank' > " +
            link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
